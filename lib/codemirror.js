...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    var instance = wrapper.CodeMirror = {
      ...
      selectWordAt: operation(selectWordAt),
    ...
    function updateVerticalScroll(scrollTop) {
      var th = textHeight(), virtualHeight = totalHeight(true), scrollbarHeight = scroller.clientHeight;
    ...
    function visibleLines(scrollTop) {
      var visibleTop = (scrollTop != null ? scrollTop : -mover.offsetTop) - paddingTop();
    ...
    function updateDisplay(changes, suppressCallback, scrollTop) {
      ...
      if (options.lineWrapping) {
        ...
        var virtualHeight = totalHeight(true), scrollbarHeight = scroller.clientHeight;
      ...
      // Update the position of inline widgets
      ...
      for (i = 0; i < inlines.length; i++) {
        ...
        if (!inline.exists()) {
          ...
          var lineNum = inline.getLineNumber();
          var pos = {line: lineNum, ch: inline.getLine().text.length - 1};
          inline.node.style.top = (paddingTop() + localCoords(pos, true).yBot) + "px";
    ...
    function patchDisplay(from, to, intact) {
      ...
      doc.iter(from, to, function(line) {
        ...
        if (!nextIntact || nextIntact.from > j) {
          ...
          var placeHolderElement = placeholderPool.pop() || document.createElement("div");
          ...
          if (inline)
            fragment.appendChild(inline.createPlaceholder());
        } else {
          if (fragment) {
    ...
    function updateGutter() {
      ...
      var html = [], lineNum = showingFrom, childIndex = 0, normalNode;
      doc.iter(showingFrom, Math.max(showingTo, showingFrom + 1), function(line) {
        if (line.hidden) {
          html.push("<pre></pre>");
        } else {
          var marker = line.gutterMarker;
          var text = options.lineNumbers ? lineNum + options.firstLineNumber : null;
          ...
          html.push("</pre>");
          if (!marker) normalNode = childIndex;
          var inline = getInlineWidgetForLine(line);
          if (inline) {
            html.push(inline.createPlaceholderHTML());
            ++childIndex;
          }
        }
        ++lineNum; ++childIndex;
      ...
      if (normalNode != null && options.lineNumbers) {
        var node = gutterText.childNodes[normalNode];
    ...
    function addInlineWidget(pos, node, initialHeight, destroyCallback) {
      ...
      mover.appendChild(inlineWidget.node);
      gutterDirty = true;
      updateDisplay(true);
    ...
    function endOperation() {
      ...
      if (newScrollPos && !preserveScrollPosOnSelChange) scrollCursorIntoView();
