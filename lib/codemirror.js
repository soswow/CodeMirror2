...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    if (osxMatch && osxMatch[1] && Number(osxMatch[1]) >= 7) {
        if (scrollbar.offsetWidth <= 1) {
          scrollbar.className += " cm-sb-overlap";
        } 
        else {
          // If the user unplugs the mouse while the scrollbars are transparent, we want
          // to make sure the scrollbar retains its width.
          scrollbar.className += " cm-sb-nonoverlap";
        }
    }
    
    // Check for IE <=8 . In that case, we need to force a width on the scrollbar as well.
    if (/MSIE [1-8]\b/.test(navigator.userAgent)) {
      scrollbar.className += " cm-sb-ie";
    ...
    // Array of inline widgets, sorted by document position
    var inlines = [], nextInlineId = 1, inlineStyleTag;
    // Pool of placeholder elements needed to batch the parsing of the HTML of the lines.
    var placeholderPool = [];
    ...
    connect(scroller, "paste", onPaste);
    ...
    var instance = wrapper.CodeMirror = {
      ...
      isDirty: function () { return (history.dirtyCounter != 0) },
      markClean: function() { history.dirtyCounter = 0; },
      ...
      addWidget: function(pos, node, scroll, vert, horiz) {
        ...
        if (vert == "over") top = pos.y;
          ...
          var vspace = Math.max(scroller.offsetHeight, totalHeight()),
      ...
      getInlineWidgetInfo: getInlineWidgetInfo,
    ...
    function isInWidget(e) {
      if (!e) return false;
      for (var n = e_target(e); n != wrapper; n = n.parentNode)
        if ((n.parentNode == code && n != mover) || n.getAttribute(inlineAttr)) return true;
      return false;
    }

    function onScroll(e) {
      if (isInWidget(e)) return;
    ...
    function onMouseDown(e) {
      ...
      var lastY = e.clientY, dirHint;
      function extend(e) {
        // If the user changes vertical mouse direction by a significant amount,
        // switch whether we push the selection above or below an inline widget.
        if (Math.abs(lastY - e.clientY) > 5) {
          dirHint = (lastY > e.clientY ? -1 : 1);
          lastY = e.clientY;
        }
        // It's okay for dirHint to start as undefined.
        var cur = posFromMouse(e, true, dirHint);
    ...
    function fastPoll() {
      ...
      p();
      // FIXME: Having a setTimeout makes the typing performance so much worse.
    ...
    function updateDisplay(changes, suppressCallback) {
      ...
      if (options.lineWrapping) {
        ...
        doc.iter(showingFrom, showingTo, function(line) {
          ...
          if (isInlinePlaceholder(curNode))
            curNode = curNode.nextSibling;
        });
        if (heightChanged)
      ...
      // Update the visibility and position of inline widgets
      var destroyedInlines = [], numVisibleInlines = 0;
      for (i = 0; i < inlines.length; i++) {
        var inline = inlines[i], lineNum, node;
        if (!inline.exists()) {
          // An edit has removed the line that this widget is attached to
          destroyedInlines.push(inline);
        }
        else {
          lineNum = inline.getLineNumber();
          node = inline.node;
          if (lineNum >= from && lineNum < to) {
            node.style.position = "absolute";
            node.style.zIndex = 10;
            // Position the widget below the bottom of the entire (wordwrapped) line.
            node.style.top = paddingTop() + localCoords({line: lineNum, ch: inline.getLine().text.length - 1}, true).yBot + "px";
      ...
      // Each visible inline adds an extra placeholder div to the set of lineDiv children.
      if (lineDiv.childNodes.length - numVisibleInlines != showingTo - showingFrom)
    ...
    function patchDisplay(from, to, intact) {
      ...
      doc.iter(from, to, function(line) {
        ...
        if (!nextIntact || nextIntact.from > j) {
          if (line.hidden) var html = "<pre></pre>";
          else {
          ...
          var placeHolderElement = placeholderPool.pop() || targetDocument.createElement("div");
          linesToUpdate.push({placeholder: placeHolderElement, html: html});
          fragment.appendChild(placeHolderElement);
            
          // If there is an inline widget attached to this line, create a placeholder in the lineDiv
          // that "props open" the space underneath the widget. The widget itself lives in a separate
          // div above the code and gutter divs. We create a placeholder in updateGutter() as well.
          var inline = getInlineWidgetForLine(line);
          if (inline)
            fragment.appendChild(inline.createPlaceholder(targetDocument));
        } else {
          if (fragment) {
      ...
      if (fragment)
        lineDiv.insertBefore(fragment, curNode);
      // Batch parsing of all the changed lines in a single call to the parser.
      // Each elment is supposed to be wrapped in a <pre> tag, so we don't have to wrap them here.
      var scratch = targetDocument.createElement("div");
      scratch.innerHTML = linesToUpdate.map(function(line) { return line.html; }).join("");
      // Iterate all the lines in order and replace the placeholders with the real lines.
      var nextParsedElement = scratch.firstElementChild;
      linesToUpdate.forEach(function(line) {
        var parsedElement = nextParsedElement;
        nextParsedElement = nextParsedElement.nextElementSibling;
        line.placeholder.parentNode.insertBefore(parsedElement, line.placeholder);
        line.placeholder.parentNode.removeChild(line.placeholder);
        // Recycle the placeholder element.
        placeholderPool.push(line.placeholder);
      });
    }
    ...
    function updateGutter() {
      ...
      doc.iter(showingFrom, Math.max(showingTo, showingFrom + 1), function(line) {
        if (line.hidden) {
          ...
          var inline = getInlineWidgetForLine(line);
          if (inline) {
            html.push(inline.createPlaceholderHTML(targetDocument));
          }
        }
        ++i;
      });
      ...
      // Skip the dummy <pre>s for hidden lines so we don't prop one open vertically
      while (firstNode && firstNode.childNodes.length == 0)
        firstNode = firstNode.nextSibling;
      if (firstNode) {
        var minwidth = String(doc.size).length, val = eltText(firstNode), pad = "";
    ...
    function moveV(dir, unit) {
      ...
      var target = coordsChar(pos.x, pos.y + dist * dir + 2, dir);
    ...
    // Inline widgets
    function addInlineWidget(pos, node, initialHeight, destroyCallback) {
      ...
      // Store a bookmark so we can track the position of the widget as the text is edited
      var bookmark = setBookmark(pos);
      if (!inlineStyleTag) {
        // Create a style tag to contain rules that set the height of the widget and
        // its placeholders
        inlineStyleTag = targetDocument.createElement('style');
        inlineStyleTag.type = "text/css";
        var head = targetDocument.getElementsByTagName("head")[0] || targetDocument.documentElement;
        head.appendChild(inlineStyleTag);
      }
      var inlineWidget = new InlineWidget(nextInlineId++, bookmark, node, inlineStyleTag.sheet, destroyCallback);
      for (i = 0; i < inlines.length; i++) {
        if (inlines[i].exists() && inlines[i].getLineNumber() > pos.line)
          break;
      }
      inlines.splice(i, 0, inlineWidget);
      if (initialHeight)
        inlineWidget.setHeight(initialHeight);
      gutterDirty = true;
      updateDisplay(true);
      scrollInlineWidgetIntoView(inlineWidget);
      return inlineWidget.id;
    }
      
    function scrollInlineWidgetIntoView(widget) {
      var inlineTop = widget.node.offsetTop + displayOffsetPx - paddingTop();
      // scroll the full widget into view unless the widget is taller than the height
      // of the editor that contains it.
      var heightForScroll = Math.min(widget.height, scroller.clientHeight);
      scrollIntoView(scroller.scrollLeft, inlineTop, scroller.scrollLeft, inlineTop + heightForScroll);
    }
    ...
    function removeInlineWidget(id, inUpdate) {
      ...
      if (i < inlines.length) {
        ...
        if (!inUpdate)
          updateDisplay(true);
        if (inlines.length == 0) {
          inlineStyleTag.parentNode.removeChild(inlineStyleTag);
          inlineStyleTag = null;
        }
      }
    }
      
    function setInlineWidgetHeight(id, height, ensureVisible) {
      ...
      if (widget) {
        ...
        updateDisplay(true);
        if (ensureVisible) {
          scrollInlineWidgetIntoView(widget);
        }
      }
    }
      
    function getInlineWidgetInfo(id) {
      var widget = getInlineWidgetById(id);
      if (widget)
        return { node: widget.node, height: widget.height, line: widget.getLineNumber() };
    }
    ...
    function coordsChar(x, y, dirHint) {
      y += scrollbar.scrollTop + mover.offsetTop;
      if (y < 0) y = 0;
      
      // Walk down the list of inlines to find the last one before the given y position.
      var th = textHeight(), inlineTop, inlineHeightSoFar = 0, heightPos;
      for (var i = 0; i < inlines.length; i++) {
        var inline = inlines[i];
        if (inline.exists()) {
          inlineTop = inlineHeightSoFar + th * heightAtLine(doc, inline.getLineNumber() + 1);
          if (inlineTop > y) {
            // We've found all the inlines before the given position.
            break;
          }
          else if (inlineTop + inline.height > y) {
            // In the middle of an inline--treat as if we're either before it or after it,
            // depending on the dirHint (if set).
            y = inlineTop; // where the next line would be if there weren't an inline editor
            if (dirHint && dirHint < 0) // moving upwards, so go to the line above the editor
              y -= th;
            break;
          }
      ...
      if (lineNo >= doc.size) {
          var lastVisibleLine = skipHidden({line: doc.size - 1, ch:0}, doc.size - 1, 0).line;
          return {line: lastVisibleLine, ch: getLine(lastVisibleLine).text.length};
      }
    ...
    function posFromMouse(e, liberal, dirHint) {
      ...
      return coordsChar(x - offL.left, y - offL.top, dirHint);
    }
  ...
  // Object that manages an inline widget. An inline widget is attached to a particular line in
  // the main editor, and pushes down the code below it. It scrolls as the editor is scrolled.
  // Implementationally, the widget node actually lives in a separate div above the gutter and
  // code divs; placeholders are inserted in the gutter and the line list to prop open the space
  // underneath.
  var inlineClassPrefix = "CM-inline", inlineAttr = "data-cm-inline", inlinePlaceholderAttr = "data-cm-placeholder";
  function InlineWidget(id, bookmark, node, styleSheet, destroyCallback) {
  	this.id = id;
    ...
    var newIndex = styleSheet.insertRule("." + this.getInlineWidgetClass() + "{height: 0}", styleSheet.cssRules.length);
    this.styleRule = styleSheet.cssRules.item(newIndex);
    this.styleSheet = styleSheet;
    
    var classes = node.getAttribute("class") || "";
    node.setAttribute("class", classes + " " + inlineClassPrefix + " " + this.getInlineWidgetClass());
    node.setAttribute(inlineAttr, id);
  }
  InlineWidget.prototype = {
    exists: function() {
      return (this.bookmark.line !== null);
    },
    ...
    getLineNumber: function() {
  	...
  	setHeight: function(height) {
  	  this.height = height;
      this.styleRule.style.height = this.height + "px";
  	},
    ...
    getInlineWidgetClass: function() {
      return inlineClassPrefix + "-" + this.id;
    },
  	
    createPlaceholder: function(targetDocument) {
      var placeholder = targetDocument.createElement("div");
      placeholder.setAttribute("class", inlineClassPrefix + " " + this.getInlineWidgetClass());
      placeholder.setAttribute(inlinePlaceholderAttr, this.id);
      return placeholder;
    },
      
    createPlaceholderHTML: function(targetDocument) {
      // Firefox doesn't support outerHTML yet, so we can't just get the HTML for the placeholder itself.
      var wrapper = targetDocument.createElement("div");
      wrapper.appendChild(this.createPlaceholder(targetDocument));
