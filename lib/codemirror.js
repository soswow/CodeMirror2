...
window.CodeMirror = (function() {
  ...
  CodeMirror.prototype = {
    ...
    totalHeight: function(includePadding) {
      var totalInlineHeight = 0, display = this.display, doc = this.view.doc;
      doc.iter(0, doc.size, function(line) {
        if (line.widgets && line.widgets.length) {
          for (var i = 0, ws = line.widgets; i < ws.length; ++i) {
            totalInlineHeight += ws[i].height;
          }
        }
      });
      var result = doc.size * textHeight(display) + totalInlineHeight;
      if (includePadding) 
        result += 2 * paddingTop(display);
      return Math.floor(result);
      //return this.view.doc.height;
    },
