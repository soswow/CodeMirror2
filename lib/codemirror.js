...
window.CodeMirror = (function() {
    ...
    if (sel.from.line == sel.to.line) {
      ...
      if (singleLine) {
        ...
        if (sel.from.ch)
          ...
          middleTop = heightAtLine(cm, fromObj);

        if (!sel.to.ch)
          middleBot = heightAtLine(cm, toObj);
        else
  ...
  function coordsCharInner(cm, lineObj, lineNo, x, y) {
    ...
    var from = lineLeft(lineObj), to = lineRight(lineObj), fromX = 0, toX;
    if (!bidi) {
  ...
  function markText(cm, from, to, options, type) {
    ...
    if (marker.className || marker.collapsed) regChange(cm, from.line, to.line + 1);
  ...
  function buildToken(builder, text, style) {
    if (!text) return;
    ...
    if (style != null || builder.measure)
      return builder.pre.appendChild(elt("span", [content], style));
    builder.pre.appendChild(content);
  }

  function buildTokenMeasure(builder, text, style) {
    for (var i = 0; i < text.length; ++i) {
      if (i && i < text.length - 1 &&
          builder.cm.options.lineWrapping &&
          spanAffectsWrapping.test(text.slice(i - 1, i + 1)))
        builder.pre.appendChild(elt("wbr"));
      builder.measure[builder.pos++] = buildToken(builder, text.charAt(i), style);
    }
  ...
  function insertLineContent(line, builder) {
    ...
    var nextChange = 0, spanStyle, collapsed;
    for (;;) {
      if (nextChange == pos) { // Update current marker set
        spanStyle = ""; collapsed = null; nextChange = Infinity;
        ...
        for (var j = 0; j < spans.length; ++j) {
          ...
          if (sp.from <= pos && (sp.to == null || sp.to > pos)) {
            ...
            if (m.collapsed && (!collapsed || collapsed.marker.width < m.width))
              collapsed = sp;
            if (sp.to != null && nextChange > sp.to) nextChange = sp.to;
      ...
      while (true) {
        if (text) {
          ...
          if (!collapsed)
            builder.addToken(builder, end > upto ? text.slice(0, upto - pos) : text, style + spanStyle);
